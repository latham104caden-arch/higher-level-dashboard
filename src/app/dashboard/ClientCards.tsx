      ...
      {clients.map(client => {
        ...
        return (
          <Link key={client.id} href={`/dashboard/${client.id}`} className="card card-hover p-6 block">
            ...
            <h2
              className="font-semibold text-base mb-1 tracking-tight"
              style={{ color: isActive ? '#F4F5F8' : '#B8BBC2' }}
            >
              {client.name}
            </h2>
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium" style={{ color: isActive ? '#5E6AD2' : '#5C606C' }}>
                {isActive ? 'View Report →' : 'View Archive →'}
              </span>
            </div>
          </Link>
        )
