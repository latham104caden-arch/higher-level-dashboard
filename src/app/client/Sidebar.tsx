...
type ClientProp = { name: string; initial?: string }

export function Sidebar({ client }: { client: ClientProp }) {
  const pathname = usePathname()
  const initial = client.initial ?? client.name.charAt(0)
  ...
  return (
    ...
    >
      <Link href="/client" className="flex items-center gap-2 mb-8 px-1">
        <Image
          src="/logo.png"
          alt="Higher Level"
          width={220}
...
export function MobileTopBar({ client }: { client: ClientProp }) {
  const pathname = usePathname()
  const initial = client.initial ?? client.name.charAt(0)
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          <div
            className="w-6 h-6 rounded-md flex items-center justify-center font-semibold text-[10px] flex-shrink-0"
            style={{ background: '#1A1B20', border: '1px solid rgba(255,255,255,0.08)', color: '#F4F5F8' }}
          >
            {initial}
          </div>
